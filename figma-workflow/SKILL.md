...
- **Token evidence**: 来自 `design-token-patch.md` 的 module-level 尺寸、间距、颜色、圆角、字号、状态样式;结构化布局必须包含列宽/项宽/行高/槽位尺寸等子项 token
...
质量通过的 generated gate 中,Token evidence 不能写 `<missing>`;缺少 module-level token evidence 或结构化布局子项 token 时,`implementation-evidence.md` 状态为 `incomplete`。
